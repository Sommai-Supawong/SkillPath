...
export default function CareerDetailPage() {
  ...
  return <section>
    ...
    <div className="card" style={{marginTop:32}}><h2>Core skill requirements</h2>
      <ul className="requirements">{career.requirements?.map(req => <li key={req.skill.id}><div><strong>{req.skill.name}</strong><div className="muted">{req.skill.category} · Difficulty {req.skill.difficulty}/5</div></div><div><span className="badge">Required {req.required_level}/5</span> <span className="badge">Importance {req.importance}/5</span></div></li>)}</ul>
    </div>
  </section>;
}
